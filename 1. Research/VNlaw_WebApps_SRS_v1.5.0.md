...
The BFF MUST implement CORS to allow requests from Netlify-hosted frontend:
- `Access-Control-Allow-Origin`: `https://vnlaw.app`, `https://*.netlify.app` (for previews)
...
```json
{
  ...
  // Note: resultCount is the TOTAL number of matches in that datastore (may be approximate),
  // not the number of results returned in this page. Use for "X results found" display.
  "warnings": ["string"],
  "auth": {
...
**Note:** `messageId` is the unique identifier for this assistant response. Use this ID when submitting feedback via `/v1/feedback`.
